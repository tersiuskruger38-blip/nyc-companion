  ...
  return (
    ...
    >
      <View style={styles.cardRow}>
        ...
        {p.status === 'visited' && <Text style={{ fontSize: 18 }} accessibilityLabel="Visited">✅</Text>}
      </View>
      {isExpanded && (
        <View style={[styles.expandedSection, { borderTopColor: theme.border }]}>
          ...
          <TouchableOpacity
            style={[styles.visitBtn, p.status === 'visited' && styles.visitedBtn]}
            onPress={onVisit}
            accessibilityRole="button"
      ...
      <FlatList
        ...
        renderItem={({ item: f }) => (
          <TouchableOpacity
            ...
            style={[styles.filterChip, { backgroundColor: theme.cardBg }, filter === f.id && { backgroundColor: theme.accent }]}
            accessibilityRole="button"
            accessibilityLabel={`Filter: ${f.label}`}
            accessibilityState={{ selected: filter === f.id }}
          >
            <Text style={[styles.filterText, { color: theme.text }, filter === f.id && { color: '#FFFFFF' }]}>{f.label}</Text>
          </TouchableOpacity>
        )}
      ...
      <FlatList
        ...
        renderItem={({ item: p }) => (
          <TouchableOpacity
            ...
            style={[styles.filterChip, { backgroundColor: theme.cardBg }, priceFilter === p.id && { backgroundColor: theme.green }]}
            accessibilityRole="button"
            accessibilityLabel={`Price filter: ${p.label}`}
            accessibilityState={{ selected: priceFilter === p.id }}
          >
            <Text style={[styles.filterText, { color: theme.text }, priceFilter === p.id && { color: '#FFFFFF' }]}>{p.label}</Text>
          </TouchableOpacity>
        )}
        ItemSeparatorComponent={() => <View style={{ width: 6 }} />}
      />
...
const styles = StyleSheet.create({
  ...
  searchInput: { padding: 12, borderRadius: 12, borderWidth: 1, fontSize: 14, marginBottom: 10 },
  filterChip: { paddingHorizontal: 12, paddingVertical: 6, borderRadius: 20 },
  filterText: { fontSize: 12, fontWeight: '600' },
  count: { fontSize: 12, marginBottom: 10 },
  card: { borderRadius: 14, padding: 12, marginBottom: 8, borderLeftWidth: 4, shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.05, shadowRadius: 2, elevation: 1 },
  ...
  visitBtn: { marginTop: 10, paddingVertical: 9, borderRadius: 10, backgroundColor: '#22C55E', alignItems: 'center' },
